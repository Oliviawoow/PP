...
(define (compute-f-with-step f a b step)
  (let calc ((x a))
    (if (> x b) null (cons (f x) (calc (+ x step))))))
...
(define (compute-perimeter points)
  (let perimeter ((current points))
    (if (null? (cdr current)) (distance (car points) (car current))
        (+ (distance (car current) (second current))
           (perimeter (cdr current))))))
