...
primes :: Int -> [Int]
primes 1 = []
-- DAR DC NU MERGE ASTA DUPA AND?????? when mod x p /= 0 (p <- primes(intSqrt x))]
...
setIntersection a b = [x | x <- a, elem x b] -- x primeste a daca apartine de b

setDiff :: Eq a => [a] -> [a] -> [a]
setDiff a b = [x | x <- a, not (elem x b)] -- x primeste a daca nu apartine de b
