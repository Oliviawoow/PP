...
(define-struct pasare (x y viteza) #:transparent)
(define-struct pipe (x y) #:transparent)
(define-struct stare (pasare pipes scor) #:transparent)
...
(define (next-state-bird bird gravity)
  (struct-copy pasare bird [y (+ (pasare-viteza bird) (get-bird-y bird))]
               [viteza (+ (pasare-viteza bird) gravity)]))
...
(define (next-state-bird-onspace bird momentum)
  (struct-copy pasare bird [viteza (- momentum)]))
...
(define (change current-state pressed-key)
  (if (key=? pressed-key " ")
      (struct-copy stare current-state
                   [pasare (next-state-bird-onspace (stare-pasare current-state) initial-momentum)])
      current-state))
...
(define (move-pipes pipes scroll-speed)
  (map (λ (pipee) (struct-copy pipe pipee [x (- (pipe-x pipee) scroll-speed)])) pipes))
...
(define (clean-pipes pipes)
  (if (<= (+ (pipe-x (car pipes)) pipe-width) 0) (cdr pipes)
      pipes))
...
(define (add-more-pipes pipes)
  (if (< (length pipes) no-pipes)
      (reverse (cons (pipe (+ pipe-width pipe-gap (pipe-x (car (reverse pipes)))) (+ added-number (random random-threshold)))
            (reverse pipes)))
            pipes))
...
(define (invalid-state? state)
  (or (check-ground-collision (get-bird state))
      (check-pipe-collisions (get-bird state) (stare-pipes state))))
...
(define (check-pipe-collisions bird pipes)
  (not (null? (filter (λ (pipee)
                        (let ([pipe-sus-sus (make-posn (pipe-x pipee) 0)]
                              [pipe-sus-jos (make-posn (+ (pipe-x pipee) pipe-width) (pipe-y pipee))]
                              [pipe-jos-sus (make-posn (pipe-x pipee) (+ (pipe-y pipee) pipe-self-gap))]
                              [pipe-jos-jos (make-posn (+ (pipe-x pipee) pipe-width) ground-y)]
                              [pasare-sus (make-posn (pasare-x bird) (pasare-y bird))]
                              [pasare-jos (make-posn (+ (pasare-x bird) bird-width) (+ (pasare-y bird) bird-height))])
                          (or (check-collision-rectangles pasare-sus pasare-jos pipe-sus-sus pipe-sus-jos)
                          (check-collision-rectangles pasare-sus pasare-jos pipe-jos-sus pipe-jos-jos))
                         )) pipes))))
...
(define (next-state state)
  (struct-copy stare state [pasare (next-state-bird (get-bird state) initial-gravity)]
               [pipes (next-state-pipes (get-pipes state) initial-scroll-speed)]
               [scor (+ (get-score state) 0.1)]))
...
(define verzisor (rectangle pipe-width pipe-height "solid" "green"))
(define albisor (rectangle (+ 10 pipe-width) pipe-self-gap "solid" "white"))
...
(define (draw-frame state)
  (place-image bird-image (+ (pasare-x (stare-pasare state)) (quotient bird-width 2)) (+ (pasare-y (stare-pasare state)) (quotient bird-height 2))
               (place-image ground-image (quotient scene-width 2) (- scene-height (quotient (- scene-height ground-y) 2))
               (place-image (score-to-image (stare-scor state)) text-x text-y
                            (place-pipes (stare-pipes state) initial-scene)))))

; Folosind `place-image/place-images` va poziționa pipe-urile pe scenă.
(define (place-pipes pipes scene)
	(foldr (λ (pipee scene) (place-image albisor (+ (pipe-x pipee) (quotient pipe-width 2)) (+ (pipe-y pipee) (quotient pipe-self-gap 2))
                                             (place-image verzisor (+ (pipe-x pipee) (quotient pipe-width 2)) (quotient pipe-height 2)
                                             scene))) scene pipes))
