...
(define (all-palindromes? n Bases)
  (cond [(null? Bases) #t]
        [else (and (palindrome? (num->base n (car Bases))) (all-palindromes? n (cdr Bases)))]))
...
(define (palindromes-to-n n Bases)
  (cond [(< n 0) '()]
        [(all-palindromes? n Bases) (append (palindromes-to-n (sub1 n) Bases) (list n))]
...
(define (first-b-pal start b)
  (cond [(first-b-pal-helper (add1 start) 2 0 b) (add1 start)]
        [else (first-b-pal (add1 start) b)]))

(define (first-b-pal-helper n base first last)
  (cond [(equal? first last) #t]
        [(> base 10) #f]
        [(palindrome? (num->base n base)) (first-b-pal-helper n (add1 base) (add1 first) last)]
        [else (first-b-pal-helper n (add1 base) first last)]))
